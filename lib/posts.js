...
export async function getImgPaths() {
    ...
    const category = fs.readdirSync(imgeDirectory, "utf8").filter(imagePath => imagePath !== '.DS_Store')
    const imgPath = {}
    for (const categoryName of category) {
        const categoryImgPath = []
        const image = fs.readdirSync(path.join(imgeDirectory, categoryName), "utf8").filter(imagePath => imagePath !== '.DS_Store')
